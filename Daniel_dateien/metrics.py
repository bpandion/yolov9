...
class ConfusionMatrix:
    ...
    def plot(self, save_dir='', names=()):
        try:
            ...
            if True:
                keep = [39, 41, 42, 43, 44, 45, 67, 68, 71, 76, 80]
                # keep = [1, 40, 42, 43, 44, 45, 46, 68, 69, 72, 77]
                non_zero_matrix = self.matrix[keep][:, keep]
                nc = 10
                names = ["bottle", "cup", "fork", "knife", "spoon", "bowl", "cell phone", "microwave", "sink", "scissors"]
                array = non_zero_matrix / (non_zero_matrix.sum(0).reshape(1, nc + 1) + 1E-6)  # normalize
...
def plot_pr_curve(px, py, ap, save_dir='pr_curve.png', names=()):
    ...
    #for j in range(len(py.mean(1))):
        #if (j+1)%10 == 0:
            #print(f"({px[j]}, {py.mean(1)[j]})")
